...
class FitDatas(object):
    """Fitting data"""
    def __init__(self, corr_results, xmin, xmax, by_p4):
        corr_map = {}
        ...
        for group, items in corr_map.items():
            xvalues, yvalues, sample_diver = prepare_fitting_data(items, xmin, xmax, by_p4)
...
def prepare_fitting_data(fitdata, xmin, xmax, by_p4):
    ...
    if by_p4:
        linearfitx = p4yvalues[:15]
        linearfity = p2yvalues[:15]
        model = sm.OLS(linearfity, linearfitx)
        results = model.fit()
        slope = results.params[0]
        xvalues = p4xvalues
        yvalues = [v * slope for v in p4yvalues]
    else:
        xvalues = p2xvalues
        yvalues = p2yvalues
...
def plot_fit(corr_results, xmin, xmax, by_p4, plot_file):
    """Fit all row data and do ploting"""
    all_data = FitDatas(corr_results, 3, 300, by_p4).get('all')
    xvalues = all_data.xvalues
    yvalues = all_data.yvalues
    sample_diversity = all_data.sample_diversity

    fit_data = FitDatas(corr_results, xmin, xmax, by_p4).get('all')
    fit_xvalues = fit_data.xvalues
    fit_yvalues = fit_data.yvalues
    ...
    fitres = fit_model1(fit_xvalues, fit_yvalues, sample_diversity)
    ...
    ax1.plot(rest_xvalues, rest_yvalues, 'o',
             ...
             markeredgecolor='k',
             markerfacecolor='None')
    predictions = yvalues + fcnmin(fitres.params, xvalues, yvalues)
...
def fitp2(corr_file, prefix, xmin, xmax, by_p4):
    """Fit p2"""
    corr_results = read_corr(corr_file)
    fitdatas = FitDatas(corr_results, xmin, xmax, by_p4)

    if fitdatas.get("all"):
        best_fit_plot_file = prefix + "_best_fit.svg"
        plot_fit(corr_results, xmin, xmax, by_p4, best_fit_plot_file)
...
def main():
    ...
    parser.add_argument('--by_p4', dest="by_p4", action='store_true')
    ...
    fitp2(datafile, prefix, xmin, xmax, by_p4)
